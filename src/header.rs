...
use libc::c_char;
use slow5lib_sys::{
    slow5_aux_add, slow5_aux_add_enum, slow5_get_aux_names, slow5_hdr_add, slow5_hdr_get,
...
impl<'a> Header<'a> {
    ...
    {
        let name = to_cstring(name)?;
        let labels: Result<Vec<CString>, Slow5Error> =
            labels.into_iter().map(|l| to_cstring(l)).collect();
        let labels = labels?;
        let num_labels = {
